...
\pause[4]
\begin{enumerate}[1.]
  \item Pick one regulator [\ex{$a$}], choose an active process for all the others [\ex{$b_0$}].
\pause[5]
  \item Compute the \tval{focal processes} when the active process changes [\ex{$a_0$}, \ex{$a_1$}].
\pause[9]
